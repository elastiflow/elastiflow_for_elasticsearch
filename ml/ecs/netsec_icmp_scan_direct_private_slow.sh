...
DETECTOR=$( cat << EOF
{
  ...
  "results_index_name": "custom-elastiflow_ecs_netsec_icmp_scan_direct_private_slow",
...
DATAFEED=$( cat << EOF
{
  "job_id": "elastiflow_ecs_netsec_icmp_scan_direct_private_slow",
  "indices": [
    "elastiflow-flow-ecs-*"
  ],
  "query": {
...
echo ""; echo "Installing anomaly_detector elastiflow_ecs_netsec_icmp_scan_direct_private_slow ..."
curl -XPUT -o /dev/null -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/anomaly_detectors/elastiflow_ecs_netsec_icmp_scan_direct_private_slow?pretty -H "Content-Type: application/json" -d "${DETECTOR}"
echo ""; echo "Installing datafeed elastiflow_ecs_netsec_icmp_scan_direct_private_slow ..."
curl -XPUT -o /dev/null -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/datafeeds/datafeed-elastiflow_ecs_netsec_icmp_scan_direct_private_slow?pretty -H "Content-Type: application/json" -d "${DATAFEED}"
