...
DETECTOR=$( cat << EOF
{
  ...
  "results_index_name": "custom-elastiflow_ecs_perf_netif_ingress_thruput_flows",
...
DATAFEED=$( cat << EOF
{
  "job_id": "elastiflow_ecs_perf_netif_ingress_thruput_flows",
  "indices": [
    "elastiflow-flow-ecs-*"
  ],
  "query": {
...
echo ""; echo "Installing anomaly_detector elastiflow_ecs_perf_netif_ingress_thruput_flows ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/anomaly_detectors/elastiflow_ecs_perf_netif_ingress_thruput_flows?pretty -H "Content-Type: application/json" -d "${DETECTOR}"
echo ""; echo "Installing datafeed elastiflow_ecs_perf_netif_ingress_thruput_flows ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/datafeeds/datafeed-elastiflow_ecs_perf_netif_ingress_thruput_flows?pretty -H "Content-Type: application/json" -d "${DATAFEED}"
