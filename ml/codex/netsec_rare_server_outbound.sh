...
DETECTOR=$( cat << EOF
{
  ...
  "description": "Rare Server (outbound)",
  "groups": [
    "elastiflow",
    "security",
    "activity"
  ...
  "analysis_config": {
    ...
    "detectors": [
      {
        "detector_description": "Rare Server",
        "function": "rare",
        "by_field_name": "flow.server.ip.addr",
        "partition_field_name": "flow.client.ip.addr",
        "detector_index": 0
      }
    ],
    "influencers": [
  ...
  "results_index_name": "custom-elastiflow_codex_netsec_rare_server_outbound",
...
DATAFEED=$( cat << EOF
{
  "job_id": "elastiflow_codex_netsec_rare_server_outbound",
  "indices": [
...
echo ""; echo "Installing anomaly_detector elastiflow_codex_netsec_rare_server_outbound ..."
curl -XPUT -o /dev/null -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/anomaly_detectors/elastiflow_codex_netsec_rare_server_outbound?pretty -H "Content-Type: application/json" -d "${DETECTOR}"
echo ""; echo "Installing datafeed elastiflow_codex_netsec_rare_server_outbound ..."
curl -XPUT -o /dev/null -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/datafeeds/datafeed-elastiflow_codex_netsec_rare_server_outbound?pretty -H "Content-Type: application/json" -d "${DATAFEED}"
