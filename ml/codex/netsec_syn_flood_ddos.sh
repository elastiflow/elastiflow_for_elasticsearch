...
DETECTOR=$( cat << EOF
{
  ...
  "analysis_config": {
    ...
    "detectors": [
      {
        "detector_description": "Excessive SYN-ACK Sessions",
    ...
    "custom_urls": [
      {
        ...
        "url_value": "dashboards#/view/a000b640-3d3e-11eb-bc2c-c5758316d788?_g=(filters:!(('\$state':(store:globalState),meta:(alias:!n,disabled:!f,index:'elastiflow-flow-codex-*',key:flow.export.type,negate:!f,params:!(ipfix,netflow),type:phrases,value:'ipfix,%20netflow'),query:(bool:(minimum_should_match:1,should:!((match_phrase:(flow.export.type:ipfix)),(match_phrase:(flow.export.type:netflow)))))),('\$state':(store:globalState),meta:(alias:!n,disabled:!f,index:'elastiflow-flow-codex-*',key:tcp.flags.bits,negate:!f,params:(query:'6'),type:phrase),query:(match_phrase:(tcp.flags.bits:'6'))),('\$state':(store:globalState),exists:(field:flow.client.as.asn),meta:(alias:!n,disabled:!f,index:'elastiflow-flow-codex-*',key:flow.client.as.asn,negate:!f,type:exists,value:exists)),('\$state':(store:globalState),meta:(alias:!n,disabled:!f,index:'elastiflow-flow-codex-*',key:flow.server.host.name,negate:!f,params:(query:'\$flow.server.host.name$'),type:phrase),query:(match_phrase:(flow.server.host.name:'\$flow.server.host.name$'))),('\$state':(store:globalState),meta:(alias:!n,disabled:!f,index:'elastiflow-flow-codex-*',key:flow.server.l4.port.name,negate:!f,params:(query:'\$flow.server.l4.port.name$'),type:phrase),query:(match_phrase:(flow.server.l4.port.name:'\$flow.server.l4.port.name$')))),refreshInterval:(pause:!t,value:0),time:(mode:absolute,from:'\$earliest$',to:'\$latest$'))"
      },
      {
        "url_name": "Threats",
...
echo ""; echo "Installing anomaly_detector elastiflow_codex_netsec_syn_flood_ddos ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/anomaly_detectors/elastiflow_codex_netsec_syn_flood_ddos?pretty -H "Content-Type: application/json" -d "${DETECTOR}"
echo ""; echo "Installing datafeed elastiflow_codex_netsec_syn_flood_ddos ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/datafeeds/datafeed-elastiflow_codex_netsec_syn_flood_ddos?pretty -H "Content-Type: application/json" -d "${DATAFEED}"
