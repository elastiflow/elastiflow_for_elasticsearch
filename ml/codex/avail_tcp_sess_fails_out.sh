...
echo ""; echo "Installing anomaly_detector elastiflow_codex_avail_tcp_sess_fails_out ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/anomaly_detectors/elastiflow_codex_avail_tcp_sess_fails_out?pretty -H "Content-Type: application/json" -d "${DETECTOR}"
echo ""; echo "Installing datafeed elastiflow_codex_avail_tcp_sess_fails_out ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/datafeeds/datafeed-elastiflow_codex_avail_tcp_sess_fails_out?pretty -H "Content-Type: application/json" -d "${DATAFEED}"
