...
DETECTOR=$( cat << EOF
{
  ...
  "results_index_name": "custom-elastiflow_codex_netsec_mdns_amplify",
...
DATAFEED=$( cat << EOF
{
  "job_id": "elastiflow_codex_netsec_mdns_amplify",
  "indices": [
...
echo ""; echo "Installing anomaly_detector elastiflow_codex_netsec_mdns_amplify ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/anomaly_detectors/elastiflow_codex_netsec_mdns_amplify?pretty -H "Content-Type: application/json" -d "${DETECTOR}"
echo ""; echo "Installing datafeed elastiflow_codex_netsec_mdns_amplify ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/datafeeds/datafeed-elastiflow_codex_netsec_mdns_amplify?pretty -H "Content-Type: application/json" -d "${DATAFEED}"
