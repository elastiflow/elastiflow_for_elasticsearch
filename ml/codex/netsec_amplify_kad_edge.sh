...
DETECTOR=$( cat << EOF
{
  ...
  "results_index_name": "custom-elastiflow_codex_netsec_amplify_kad_edge",
...
DATAFEED=$( cat << EOF
{
  "job_id": "elastiflow_codex_netsec_amplify_kad_edge",
  "indices": [
...
echo ""; echo "Installing anomaly_detector elastiflow_codex_netsec_amplify_kad_edge ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/anomaly_detectors/elastiflow_codex_netsec_amplify_kad_edge?pretty -H "Content-Type: application/json" -d "${DETECTOR}"
echo ""; echo "Installing datafeed elastiflow_codex_netsec_amplify_kad_edge ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/datafeeds/datafeed-elastiflow_codex_netsec_amplify_kad_edge?pretty -H "Content-Type: application/json" -d "${DATAFEED}"
