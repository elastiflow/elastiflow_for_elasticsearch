...
DETECTOR=$( cat << EOF
{
  ...
  "results_index_name": "custom-elastiflow_flow_netif_egress_throughput",
...
DATAFEED=$( cat << EOF
{
  "job_id": "elastiflow_flow_netif_egress_throughput",
  "indices": [
    "elastiflow-flow-ecs-*"
  ],
  "query": {
...
echo ""; echo "Installing anomaly_detector elastiflow_flow_netif_egress_throughput ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/anomaly_detectors/elastiflow_flow_netif_egress_throughput?pretty -H "Content-Type: application/json" -d "${DETECTOR}"
echo ""; echo "Installing datafeed elastiflow_flow_netif_egress_throughput ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/datafeeds/datafeed-elastiflow_flow_netif_egress_throughput?pretty -H "Content-Type: application/json" -d "${DATAFEED}"
