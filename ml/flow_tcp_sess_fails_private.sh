...
DETECTOR=$( cat << EOF
{
  ...
  "results_index_name": "custom-elastiflow_flow_tcp_sess_fails_private",
...
DATAFEED=$( cat << EOF
{
  "job_id": "elastiflow_flow_tcp_sess_fails_private",
  "indices": [
    "elastiflow-flow-ecs-*"
  ],
  "query": {
...
echo ""; echo "Installing anomaly_detector elastiflow_flow_tcp_sess_fails_private ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/anomaly_detectors/elastiflow_flow_tcp_sess_fails_private?pretty -H "Content-Type: application/json" -d "${DETECTOR}"
echo ""; echo "Installing datafeed elastiflow_flow_tcp_sess_fails_private ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/datafeeds/datafeed-elastiflow_flow_tcp_sess_fails_private?pretty -H "Content-Type: application/json" -d "${DATAFEED}"
