...
DETECTOR=$( cat << EOF
{
  ...
  "results_index_name": "custom-elastiflow_netsec_icmp_flood_direct",
...
DATAFEED=$( cat << EOF
{
  "job_id": "elastiflow_netsec_icmp_flood_direct",
  "indices": [
    "elastiflow-flow-ecs-*"
  ],
  "query": {
...
echo ""; echo "Installing anomaly_detector elastiflow_netsec_icmp_flood_direct ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/anomaly_detectors/elastiflow_netsec_icmp_flood_direct?pretty -H "Content-Type: application/json" -d "${DETECTOR}"
echo ""; echo "Installing datafeed elastiflow_netsec_icmp_flood_direct ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/datafeeds/datafeed-elastiflow_netsec_icmp_flood_direct?pretty -H "Content-Type: application/json" -d "${DATAFEED}"
