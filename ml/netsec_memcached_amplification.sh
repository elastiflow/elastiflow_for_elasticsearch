...
DETECTOR=$( cat << EOF
{
  ...
  "results_index_name": "custom-elastiflow_netsec_memcached_amplification",
...
DATAFEED=$( cat << EOF
{
  "job_id": "elastiflow_netsec_memcached_amplification",
  "indices": [
    "elastiflow-flow-ecs-*"
  ],
  "query": {
...
echo ""; echo "Installing anomaly_detector elastiflow_netsec_memcached_amplification ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/anomaly_detectors/elastiflow_netsec_memcached_amplification?pretty -H "Content-Type: application/json" -d "${DETECTOR}"
echo ""; echo "Installing datafeed elastiflow_netsec_memcached_amplification ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/datafeeds/datafeed-elastiflow_netsec_memcached_amplification?pretty -H "Content-Type: application/json" -d "${DATAFEED}"
