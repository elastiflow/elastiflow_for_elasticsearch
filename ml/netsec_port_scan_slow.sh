...
DETECTOR=$( cat << EOF
{
  ...
  "results_index_name": "custom-elastiflow_netsec_port_scan_slow",
...
DATAFEED=$( cat << EOF
{
  "job_id": "elastiflow_netsec_port_scan_slow",
  "indices": [
    "elastiflow-flow-ecs-*"
  ],
  "query": {
...
echo ""; echo "Installing anomaly_detector elastiflow_netsec_port_scan_slow ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/anomaly_detectors/elastiflow_netsec_port_scan_slow?pretty -H "Content-Type: application/json" -d "${DETECTOR}"
echo ""; echo "Installing datafeed elastiflow_netsec_port_scan_slow ..."
curl -XPUT -u ${USERNAME}:${PASSWORD} -k ${ES_HOST}/_ml/datafeeds/datafeed-elastiflow_netsec_port_scan_slow?pretty -H "Content-Type: application/json" -d "${DATAFEED}"
